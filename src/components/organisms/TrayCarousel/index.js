...
import { TrayCarouselArrow, TrayCarouselDots } from "../../../components";

const TrayCarouselWapper = styled.div`
  position: relative;
  background: #f9f9f9;
`;
const TrayCarouselFrameWrapper = styled.div`
  width: 100%;
  overflow: hidden;
`;
const ULClass = {
...
const SpanWrapper = styled.span`
  ...
  color: #888;
`;
const TrayCarouselItem = styled.li`
  img {
    width: 100%;
  }
`;
  ...
  onSwipeEnd = () => {
    ...
    if (swipeLength !== 0 && swipeLength > this.state.slidesWidth / 2) {
      ...
      if (this.state.touchObject.direction === 1) {
        ...
        currentIndex = this.getTargetIndex(
          targetIndex,
  ...
  getFormatedChildren = (children, lazyLoadedList, visibleSlideList) =>
    Children.map(children, (child, index) => {
      ...
      if (!settings.lazyLoad || lazyLoadedList.indexOf(index) >= 0) {
        return (
          <TrayCarouselItem key={index} style={this.getSlideStyles(isVisible)}>
            {child}
          </TrayCarouselItem>
        );
      }
  ...
  render() {
    ...
    if (settings.arrows && !this.state.singlePage && !scrollOnDevice) {
      if (settings.prevArrow == null) {
        prevArrow = (
          <TrayCarouselArrow next={false} onClick={this.moveToPrevious} />
        );
      } else {
      ...
      if (settings.nextArrow == null) {
        nextArrow = <TrayCarouselArrow onClick={this.moveToNext} />;
      } else {
    ...
    if (settings.paging && !this.state.singlePage && !scrollOnDevice) {
      dots = (
        <SpanWrapper>
          {`${this.state.activePage + 1} ${this.props.pagingSeparator} ${
            this.state.slidePages
            }`}
        </SpanWrapper>
      );
    }
    ...
    if (this.props.scrollOnDevice && isTouchDevice()) {
      trackStyles = Object.assign({}, ULClass, this.getScrollTrackStyles);
    } else {
    ...
    return (
      ...
      >
        {prevArrow}
        <TrayCarouselFrameWrapper ref={this.storeFrameRef}>
          <ul
            style={trackStyles}
            onMouseDown={!disableSwipeEvents ? this.onSwipeStart : null}
            onMouseLeave={
              this.state.dragging || !disableSwipeEvents
                ? this.onSwipeEnd
                : null
            }
            onMouseMove={
